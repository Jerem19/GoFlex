...
<div class="row mt col-lg-12 form-panel" style="margin-bottom: 40px;">
    <form class="form-horizontal style-form" id="formUpdateProfile" method="post">
        ...
        <input class="col-sm-8 form-control" disabled="disabled" value="<?= $user->getFirstname() ?>" name="firstname" />

        <label class="control-label col-sm-4"><?= L10N['index']['profile']['lastname']?></label>
        <input class="col-sm-8 form-control" disabled="disabled" value="<?= $user->getLastname() ?>" name="lastname" />

        <label class="control-label col-sm-4"><?= L10N['index']['profile']['phone']?></label>
        <input class="col-sm-8 form-control" type="phone" value="<?= $user->getPhone() ?>" name="phone"/>

        <label class="control-label col-sm-4"><?= L10N['index']['profile']['email']?></label>
        <input disabled="disabled" class="col-sm-8 form-control" value="<?= $user->getEmail() ?>" name="email" />

        <label class="control-label col-sm-4"><?= L10N['index']['profile']['username']?></label>
        <input style="margin-bottom: 20px;" disabled="disabled" class="col-sm-8 form-control" value="<?= $user->getUsername() ?>" name="username" />
            ...
            $.post("updateProfile", att, function (data) {
                $.gritter.add({
                    text: JSON.parse(data) ?
                        "<?= L10N['index']['profile']['alertUpdateUserSuccess'] ?>" : "<?= L10N['index']['profile']['alertUpdateUserFailed'] ?>"
                });
            });
            return false;
        });
